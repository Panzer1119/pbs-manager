...
export class ArchiveService {
    ...
    async parseArchiveIndexes(): Promise<void> {
        await this.dataSource.transaction(async (transactionalEntityManager: EntityManager): Promise<void> => {
            ...
            const namespaceIds: Set<number> = new Set(groups.map(group => group.namespaceId).filter(id => id != null));
